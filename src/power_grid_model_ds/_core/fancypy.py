...
def concatenate[T: FancyArray](fancy_array: T, *other_arrays: T | np.ndarray) -> T:
    """Concatenate arrays."""
...
def sort[T: FancyArray](array: T, axis=-1, kind=None, order=None) -> T:
    """Sort the array in-place and return sorted array."""
    array.data.sort(axis=axis, kind=kind, order=order)
    return array


def array_equal[T: FancyArray](array1: T, array2: T, equal_nan: bool = True) -> bool:
