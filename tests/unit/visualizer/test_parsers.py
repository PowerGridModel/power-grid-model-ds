...
def node_array() -> tuple[NodeArray, VizToComponentElements]:
    ...
    expected_elements = {
        str(nodes["id"][i]): {
            "data": {"id": str(nodes["id"][i]), "group": "node"},
...
def branch3_array() -> tuple[Branch3Array, VizToComponentElements]:
    ...
    expected_elements = {
        "200_0": {
            "data": {"id": "200_0", "group": "three_winding_transformer", "source": "1", "target": "2"},
            "classes": f"{StyleClass.BRANCH.value} {StyleClass.TRANSFORMER.value}",
        },
        "200_1": {
            "data": {"id": "200_1", "group": "three_winding_transformer", "source": "1", "target": "3"},
            "classes": f"{StyleClass.BRANCH.value} {StyleClass.TRANSFORMER.value}",
        },
        "200_2": {
            "data": {"id": "200_2", "group": "three_winding_transformer", "source": "2", "target": "3"},
            "classes": f"{StyleClass.BRANCH.value} {StyleClass.TRANSFORMER.value}",
        },
    }
